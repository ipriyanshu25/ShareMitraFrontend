...
const PaymentPage: React.FC = () => {
    ...
    if (ifsc !== "") {
      try {
        ...
        if (response.ok) {
          ...
          setBankDetails((prev) => ({ ...prev, bankName: "" }));
          Swal.fire({ title: "Error", text: "Invalid IFSC Code", icon: "error", confirmButtonText: "OK" });
        }
      } catch (error) {
        setBankDetails((prev) => ({ ...prev, bankName: "" }));
        Swal.fire({ title: "Error", text: "Error fetching bank details", icon: "error", confirmButtonText: "OK" });
      }
    }
  };
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    if (paymentMethod === "bank") {
      ...
      if (!accountNumberRegex.test(bankDetails.accountNumber)) {
        Swal.fire({ title: "Error", text: "Invalid account number.", icon: "error", confirmButtonText: "OK" });
        return;
      }
      if (!bankDetails.accountHolder.trim() || !bankDetails.ifsc.trim() || !bankDetails.bankName.trim()) {
        Swal.fire({ title: "Error", text: "Please fill all bank details.", icon: "error", confirmButtonText: "OK" });
        return;
      }
    } else {
      const upiRegex = /^[\w.-]+@[\w.-]+$/;
      if (!upiRegex.test(upiDetails.upiId.trim())) {
        Swal.fire({ title: "Error", text: "Invalid UPI ID.", icon: "error", confirmButtonText: "OK" });
        return;
      }
    }
    ...
    try {
      ...
      if (response.ok) {
        Swal.fire({ title: "Success", text: data.msg, icon: "success", confirmButtonText: "OK" });
        setEditingPayment(null);
        setBankDetails({ accountHolder: "", accountNumber: "", ifsc: "", bankName: "" });
        setUpiDetails({ upiId: "" });
        fetchPaymentMethods(userId);
      ...
      Swal.fire({ title: "Error", text: "Submission failed.", icon: "error", confirmButtonText: "OK" });
    }
  };
